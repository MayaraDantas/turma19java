		...
		do {
			...
			if (opcao == 1) {
				if (confirma1 == true) { // Confirma se o produto já foi adquirido
					System.out.printf(
							"Time: %s | Valor Unitário: R$ %.2f | Marca: %s | Quantidade de itens no estoque: (%d)",
				...
				if (confirma1 == false || confirma2 == false || confirma3 == false || confirma4 == false
						|| confirma5 == false) {
					...
					do {
						System.out
								.print("Forma de pagamento: 1 - Dinheiro 2 - Cartão de Débito 3 - Cartão de Crédito: ");
						opcao = leia.nextInt();
					...
					if (opcao == 1) {
						...
						System.out.printf("Imposto: R$ %.2f%nDesconto: R$ %.2f%nTotal Compra: R$ %.2f%n", imposto,
								desconto, total);
					} else if (opcao == 2) {
						...
						if (opcao == 1) {
							...
							total = total + juros10;
							System.out.printf(
									"Imposto: R$ %.2f%nJuros: R$ %.2f%nValor Parcela: R$ %.2f%nTotal Compra: R$ %.2f%n",
